...
from entity.MutantExecInfo import MutantExecInfo


class NewMutationExecutorV3:
    ...
    def __exec_mutant_fail(self, model, m_info: MutantExecInfo):
        n_f2p = 0
        n_i_f = 0
        ...
        m_info.upper_sus = sus0
        #填充失败信息
        m_info.set_f_info(n_f2p,self.__failing_test_inputs - n_f2p)
                ...
                for compressed_mutant in compressed_mutants:
                    ...
                    try:
                        with tf.device('/CPU:0'):
                            cur_mutant = MutantExecInfo(mutant_name)
                            with arc.extractfile(mutant_name) as f:
                                file_obj = io.BytesIO(f.read())
                                with h5py.File(file_obj, 'r') as h5f:
                                    model = load_model(h5f)
                            self.__exec_mutant_fail(model, cur_mutant)
                            cur_mutant_list.append(cur_mutant)
                            '''删除模型相关的代码
                            '''
                            import gc
                            # 删除模型
                    ...
                    # 对变异体列表重新排序
                sorted_mutants_desc = sorted(cur_mutant_list, key=lambda x: x.upper_sus, reverse=True)
                for sorted_mutant in sorted_mutants_desc:
                    cur_mutant_name = sorted_mutant.getName()
                    if sorted_mutant.upper_sus <= self.cur_layer_sus:
                        break
                    try:
                        with tf.device('/CPU:0'):
                            with arc.extractfile(cur_mutant_name) as f:
                                file_obj = io.BytesIO(f.read())
                                with h5py.File(file_obj, 'r') as h5f:
                                    model = load_model(h5f)
                            self.__exec_mutant_pass(model, sorted_mutant)
                            self.cur_layer_sus = max(self.cur_layer_sus,sorted_mutant.real_sus)
                    except:
                        print('Non-viable mutant ' + cur_mutant_name)
                        print('Exception raised: {}'
                              ''.format(error))
                    finally:
                        if os.path.exists(cur_mutant_name):
                            os.remove(cur_mutant_name)
