...
class MutationGenerator:
    def __init__(self, filename):
        mut_dir = mut_file_path.split('.')[0]
        if os.path.isdir(mut_dir):
            shutil.rmtree(mut_dir)
        if os.path.isfile(mut_file_path):
            os.remove(mut_file_path)
        self._tar_workdir = tarfile.open(mut_file_path, 'w:gz')
        # 使用E盘写法
        # mut_dir=os.path.join('E://','workdir')
        # mut_dir_file=os.path.join('E://','workdir.tar.gz')
        # if os.path.isdir(mut_dir):
        #     shutil.rmtree(mut_dir)
        # if os.path.isfile(mut_dir_file):
        #     os.remove(mut_dir_file)
        # self._tar_workdir = tarfile.open(mut_dir_file, 'w:gz')
    ...
    def apply_math_weight(self):
        for layer_no in range(0, len(self._model.layers)):
            ...
            for neuron_no in range(0, layer.units):
                ...
                self._submit_model(v_path)
                end_time=time.time()
                # self.runCal.cal_time(layer_no,neuron_no,'MATH_WEIGHT_4',end_time-start_time)
                weights[0][:, neuron_no] *= 2.0
                layer.set_weights(weights)  # restore the model
