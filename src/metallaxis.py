    ...
    def calculate_type1_scores(self):
        ...
        for (layer_id, layer_mutation_exec_info) in self.__mutation_testing_results__.items():
            ...
            for mutant_name,result in layer_mutation_exec_info.get_run_dict().items():
                # layer_idx,neuron_idx,mutant_oper = MutantNameParser(mutant_name)
                n_i_f,n_i_p=result[0],result[1]
                if n_i_f + n_i_p == 0 or self.__tot_f == 0:
                    sbi_score = 0
                    ochiai_score = 0
                else:
                ...
                sbi_score_sum = sbi_score_sum + sbi_score
                ochiai_score_sum = ochiai_score_sum + ochiai_score
                # if mutant_oper.endswith("model.h5"):
                #     mutant_oper=mutant_oper[:-8]
                # if layer_idx!=-1:
                #     # self.__mutants_sus_scores[mutant_Tuple].append(sbi_score)
                #     self.__mutants_sus_scores[layer_idx][neuron_idx][mutant_oper]["sbi_score"]=sbi_score
                #     self.__mutants_sus_scores[layer_idx][neuron_idx][mutant_oper]["ochiai_score"]=ochiai_score
                n = n + 1
